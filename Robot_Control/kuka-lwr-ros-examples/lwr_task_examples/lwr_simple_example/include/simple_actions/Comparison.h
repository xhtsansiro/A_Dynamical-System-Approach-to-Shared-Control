#ifndef KUKA_LWR_COMPARISON_H
#define KUKA_LWR_COMPARISON_H
...
class Comparison_action : public ros_controller_interface::Ros_ee_j, public ac::Base_action {   //inherit from Base_action, two methods bool update() and bool stop()

public:
    

    Comparison_action(ros::NodeHandle& nh);  //
...
private:
    ...
    std::unique_ptr<motion::CDDynamics> linear_cddynamics;   // unique_ptr 是智能指针, 类型是对象, motion空间下的CDDDynamics
    std::unique_ptr<motion::CDDynamics> angular_cddynamics;

    ros_controller_interface::Switch_controller switch_controller; // 请求调用服务，有switch controller 和 controller list.
